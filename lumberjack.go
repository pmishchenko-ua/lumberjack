...
// is less than MaxSize bytes, lumberjack will open and append to that file.
...
// - a write would cause the log file to be larger than MaxSize, or
// - the file has been created more than MaxLifetime seconds ago
// the file is closed, renamed to include a timestamp of the
...
func backupName(name string, local bool) string {
	...
	return filepath.Join(dir, fmt.Sprintf("%s_%s", filename, t.Format(backupTimeFormat)))
}

// timeFromName extracts the formatted time from the filename by stripping off
// the filename's prefix. timeFromName is basicakky the inverse function of backupName
func (l *Logger) timeFromName(filename string) (time.Time, error) {
	basename := filepath.Base(l.filename())

	ts := strings.TrimSuffix(filename, compressSuffix)
	ts = strings.TrimPrefix(ts, basename)
	ts = strings.TrimPrefix(ts, "_")
...
func (l *Logger) oldLogFiles() ([]logInfo, error) {
	...
	for _, f := range files {
		...
		if t, err := l.timeFromName(f.Name()); err == nil {
